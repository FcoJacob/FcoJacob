...
<template>
  <div>
    <div
      v-if="isNestedLabsRoute"
      class="labs-nested-route"
      :class="{ 'labs-nested-route-showroom': isShowroomRoute }"
    >
      <NuxtPage />
    </div>
    ...
    <div v-else class="space-y-12">
      ...
      <UCard v-reveal variant="subtle" class="featured-lab-card overflow-hidden">
        <div class="grid gap-6 lg:grid-cols-[1.25fr_0.9fr] lg:items-center">
          <div class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
              <UBadge
                v-for="tag in [
                  t('labs_showroom.tag_webgl'),
                  t('labs_showroom.tag_scroll'),
                  t('labs_showroom.tag_configurator'),
                ]"
                :key="tag"
                :label="tag"
                color="neutral"
          ...
          <div
            class="relative min-h-64 overflow-hidden rounded-2xl border border-white/10 bg-black/30"
          >
            <img
            ...
            />
            <div
              class="absolute inset-0 bg-linear-to-t from-black/60 via-black/15 to-transparent"
            />
...
.featured-lab-card {
  ...
  background:
    radial-gradient(
      circle at top left,
      color-mix(in oklab, var(--ui-color-primary-500) 20%, transparent),
      transparent 40%
    ),
    linear-gradient(
      135deg,
      color-mix(in oklab, var(--ui-bg-elevated) 92%, black 8%),
      color-mix(in oklab, var(--ui-bg) 82%, black 18%)
    );
}
</style>
